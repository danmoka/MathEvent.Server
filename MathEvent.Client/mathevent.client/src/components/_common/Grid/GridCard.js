...
import { cropText, splitText } from '../../../utils/text';
...
const GridCard = ({
  ...
  const { text: originalPrimaryText, croppedText: croppedPrimaryText } =
    cropText(primaryTextLength, primaryText || '');
  const splittedAdditionalInfo = splitText(
  ...
  return (
    ...
    >
      <CardHeader
        ...
        title={croppedPrimaryText || originalPrimaryText}
      ...
      />
      <CardMedia title={primaryText}>
        <div className="grid-card__box">
          <Image className="grid-card__image" src={image} alt={primaryText} />
        </div>
      </CardMedia>
      <CardContent>
        <Typography variant="body2" color="textSecondary" component="p">
          {splittedAdditionalInfo}
        </Typography>
      </CardContent>
    </Card>
  );
